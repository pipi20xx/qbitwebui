...
const TABS: { id: SettingsTab; label: string; icon: ReactNode }[] = [
	...
	},
	{
		id: 'rss',
		label: 'RSS',
		icon: <path strokeLinecap="round" strokeLinejoin="round" d="M12.75 19.5v-.75a7.5 7.5 0 0 0-7.5-7.5H4.5m0-6.75h.75c7.87 0 14.25 6.38 14.25 14.25v.75M6 18.75a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />,
	},
	{
		id: 'webui',
		label: 'WebUI',
...
export function SettingsPanel({ instance, onClose }: Props) {
	...
	return (
		<div className="mb-4 rounded-lg border overflow-hidden" style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border)' }}>
			<div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-tertiary)' }}>
				<div className="flex items-center gap-3">
					...
					<span className="text-xs" style={{ color: 'var(--text-muted)' }}>— {instance.label}</span>
				</div>
				<div className="flex items-center gap-2">
					{hasChanges && (
						<span className="text-[10px] px-1.5 py-0.5 rounded" style={{ backgroundColor: 'color-mix(in srgb, var(--warning) 20%, transparent)', color: 'var(--warning)' }}>
							unsaved
						</span>
					)}
					<button type="button" onClick={handleClose} className="p-1 rounded hover:bg-[var(--bg-primary)]" style={{ color: 'var(--text-muted)' }}>
			...
			<div className="flex border-b overflow-x-auto" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-primary)' }}>
				{TABS.map(tab => (
					<button
						type="button"
						key={tab.id}
			...
			{loading ? (
				...
				<div className="flex items-center justify-center py-12 text-xs" style={{ color: 'var(--error)' }}>{error}</div>
			) : (
				<>
					...
					<div className="flex items-center gap-2 px-4 py-3 border-t" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--bg-tertiary)' }}>
						{error && <span className="text-xs mr-auto" style={{ color: 'var(--error)' }}>{error}</span>}
						<button
							type="button"
							onClick={handleSave}
